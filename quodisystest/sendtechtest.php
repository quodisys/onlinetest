...
$sqlt = "select * from topics where id in (".$position.")";
...
$actual_link = 'https://'. $_SERVER['HTTP_HOST'] .'/'. 'technicaltest/?token=' . trim($token).'&ca='.$id.'&cl='.$_SESSION["userid"].'&p='.$position.'#/login/'.$position;
...
<table border="0" cellpadding="0" cellspacing="0" align="center" bgcolor="#FFFFFF" width="560" style="border-collapse: collapse; border-spacing: 0; padding: 0; width: inherit; max-width: 855px; margin-bottom: 100px;" class="container">
	...
	<tr><td align="left" valign="top" style="border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0; padding-left: 10%; padding-right: 3.25%; width: 87.5%; font-size: 16px; font-weight: 400; line-height: 160%; padding-top: 10px; color: #4a4a4a; font-family: sans-serif;" class="paragraph">
		...
		<p style="padding-right:30px;">Congratulation for completing the Preliminary Tests successfully. Next step, you will take The Technical Test. Please follow given URL & login detail:</p>
		...
		<ol style="padding-left: 22px;">
			<li>Test question content relevant tech-stacks issues.</li>
...
Sendmail::send($user_email, $company." - Technical test", $content, false);
//Csvmanager::setfortechtest($id, $user_email, $new_password);

$sql = "update ".$_SESSION["keyword"]."_candidates set password='$new_password', testsent='techqds', passwordused=0 where id = ".$id;
$rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));

$sql = "insert into ".$_SESSION["keyword"]."_candidates_history (candidate, client, status, createddate) values(".$id.", ".$_SESSION['userid'].", '".$topic." - Tech Test Sent', '".date('Y-m-d H:i:s')."')";
$rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));
