	...
	<div class="container">
		<div class="row">
			<div class="div2 gradient">
				...
				<?php
				//mysqli_query($conn, "select concat('select * from ', group_concat(concat(keyword,'_candidates'))) from clients into @myVar;");
				mysqli_query($conn, "
				select 
					concat('select * from ', 
						group_concat(concat(keyword,'_candidates, '), concat(keyword,'_candidates_history'))
					)
				from clients into @myVar;");
				mysqli_query($conn, "prepare stmt from @myVar;");
				$queryResult = mysqli_query($conn, "execute stmt;");
				...
				if($company!="") {
					...
					//mysqli_query($conn, "select concat('select * from ', group_concat(concat(keyword,'_candidates'))) from clients into @myVar;");
					mysqli_query($conn, "prepare stmt from @myVar;");
					$rs = mysqli_query($conn, "execute stmt;");
					mysqli_query($conn, "deallocate prepare stmt;");
				}
				$totalrecords = mysqli_num_rows($rs);
				?>
					...
					<td style="width: 100%;" colspan="5">
						<div class="expand">
							<table cellpadding="10" width="100%">
								<tbody>
									<tr>
										<td class="tableheadbg">Apply for Company:</td>
										<td><?=$row['company'];?></td>
										<td class="tableheadbg">Position:</td>
										<td><?=$row['position'];?></td>
										<td class="tableheadbg">DOB:</td>
										<td><?=$row['dob'];?></td>
									</tr>
									<tr>
